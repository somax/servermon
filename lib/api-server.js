...
var _handles = {
	...
	'/download/db.json':function(req,res){
		var filename = config.storage.jsonFile || 'db/db.json';
		try{
			res.writeHead(200,{'Content-disposition': 'attachment; filename=db.json','Content-type': MimeMap.json});
		...
		switch (req.method){
			...
			case 'DELETE':
				if(_q){
					...
					var _warningCount = storage.get('warningCount');
					if(_warningCount.hasOwnProperty(_q)){
						delete _warningCount[_q];
						storage.set('warningCount',_warningCount);
					}
...
function start () {
	...
	log('API Server start at http://127.0.0.1:1338/');
}
